...
export function TradeCreateShell({ options, defaultAccountId, userName, userEmail }: TradeCreateShellProps) {
	return (
		<TradePageShell title="New Trade" accounts={options.accounts} activeAccountId={defaultAccountId} userName={userName} userEmail={userEmail}>
			...
			<TradeForm mode="create" options={options} defaultValues={getTradeFormDefaults(null, defaultAccountId)} />
		</TradePageShell>
	);
}
